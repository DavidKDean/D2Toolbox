...
function formatResults(xml) {
	...
	var table =
		'<tr><th>Name</th><th>Bases</th><th>Rune Order</th><th>Stats</th></tr></table>';
	...
	for (var i = 0; i < x.length; i++) {
		...
		for (var j = 0; j < stats.length; j++) {
			for (var k = 0; k < stats[j].children.length; k++) {
				table += stats[j].children[k].childNodes[0].nodeValue + '<br />';
			}
		}
...
function strFilter(rwList) {
	...
	if (searchStr != null && searchStr != '') {
		for (var i = 0; i < rwList.length; i++) {
			if (
				rwList[i].innerHTML.toLowerCase().search(searchStr.toLowerCase()) != -1
			) {
...
function filterBySockets(rwList) {
	...
	if (document.getElementById('anySockets').checked) {
		...
		for (var i = 0; i < rwList.length; i++) {
			var sockets = rwList[i]
				.getElementsByTagName('BaseSockets')[0]
				.childNodes[0].nodeValue.trim();
			if (document.getElementById('twoSockets').checked && sockets == 2) {
				filteredList[z] = rwList[i];
				z++;
			} else if (
				document.getElementById('threeSockets').checked &&
				sockets == 3
			) {
				filteredList[z] = rwList[i];
				z++;
			} else if (
				document.getElementById('fourSockets').checked &&
				sockets == 4
			) {
				filteredList[z] = rwList[i];
				z++;
			} else if (
				document.getElementById('fiveSockets').checked &&
				sockets == 5
			) {
				filteredList[z] = rwList[i];
				z++;
			} else if (
				document.getElementById('sixSockets').checked &&
				sockets == 6
			) {
				filteredList[z] = rwList[i];
				z++;
